...
public class ReimbursementRequestCache {
    ...
    private void load(){
        ...
        try {
            ...
            while (rs.next()) {
                ...
                ReimbursementRequest tempRR = new ReimbursementRequest(rrId, rrEmployeeID, rrTypeID, rrNote, rrAmount, rrDate, isAccepted);
    ...
    public boolean updateReimbursementRequestTrue(ReimbursementRequest reimbursementRequest){
        Boolean updateStatus = DatabaseConnection.getInstance().updateData(DBQueriesConstant.UPDATE_REIMBURSEMENT_REQUESTS_TRUE_QUERY +"\""+reimbursementRequest.getRrId()+"\";");
        if(updateStatus){
    ...
    public boolean updateReimbursementRequestFalse(ReimbursementRequest reimbursementRequest){
        Boolean updateStatus = DatabaseConnection.getInstance().updateData(DBQueriesConstant.UPDATE_REIMBURSEMENT_REQUESTS_FALSE_QUERY +"\""+reimbursementRequest.getRrId()+"\";");
        if(updateStatus){
