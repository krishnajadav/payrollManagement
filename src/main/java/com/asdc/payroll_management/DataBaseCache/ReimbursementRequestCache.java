...
public class ReimbursementRequestCache {
    ...
    private void load(){
        ...
        try {
            ...
            while (rs.next()) {
                ...
                int RR_Amount = rs.getInt("RR_Amount");
                Date RR_Date = rs.getDate("RR_Date");
                String isAccepted = rs.getString("isAccepted");
                ReimbursementRequest tempRR = new ReimbursementRequest(RR_ID, RR_EmployeeID, RR_TypeID, RR_Note, RR_Amount, RR_Date, isAccepted);
    ...
    public  HashMap<String, ReimbursementRequest> getAllReimbursements(){
    ...
    public boolean updateReimbursementRequestTrue(ReimbursementRequest e){
        Boolean updateStatus = DatabaseConnection.getInstance().updateData(DBQueriesConstant.updateReimbursementRequestsTrueQuery+"\""+e.getRR_ID()+"\";");
        if(updateStatus){
            e.setIsAccepted("1");
    ...
    public boolean updateReimbursementRequestFalse(ReimbursementRequest e){
        Boolean updateStatus = DatabaseConnection.getInstance().updateData(DBQueriesConstant.updateReimbursementRequestsFalseQuery+"\""+e.getRR_ID()+"\";");
        if(updateStatus){
            e.setIsAccepted("0");
            modelReimbursementRequests.put(e.getRR_ID(),e);
        }
