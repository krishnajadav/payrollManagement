    ...
    {	
		try {
	        ...
	        for (Map.Entry mapElement : employeeHashMap.entrySet()) {
	            ...
	            if (employeeTemp.getEmployee_emailID().equalsIgnoreCase(ua.getUserEmail()) && employeeTemp.getEmployee_Password().equals(ua.getEncriptedPassword(ua.getUserPassword())))
	            {	            
	                return employeeTemp.getEmployee_ID()+"#"+ employeeTemp.getEmployee_Name() +"#"+ employeeTemp.getAccess_level();
	            }
	        }
	        return "Invalid User";
		}
